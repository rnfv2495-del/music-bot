...
def search_all():
    ...
    try:
        token = get_spotify_token()
        print(f"[search] spotify token: {token[:20] if token else 'None'}")
        res = requests.get('https://api.spotify.com/v1/search',
            headers={'Authorization': f'Bearer {token}'},
        ...
        print(f'[search] spotify results: {len([x for x in merged if x["source"]=="sp"])}')
    except Exception as e:
    ...
    try:
        ...
        print(f'[search] sc results: {len([x for x in merged if x["source"]=="sc"])}')
    except Exception as e:
